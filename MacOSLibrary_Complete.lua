...
function MacOSLib:CreateWindow(config)
    ...
    local configFileName = config.ConfigFileName
    
    -- ปรับขนาดอัตโนมัติตามอุปกรณ์
    local windowSize
    if isMobile() then
        windowSize = UDim2.new(0.95, 0, 0.85, 0) -- 95% ของหน้าจอสำหรับมือถือ
    else
        windowSize = config.Size or UDim2.new(0, 900, 0, 600)
    end
    ...
    -- Sidebar Header (ส่วนบนของ sidebar - สีเทา)
    ...
    -- Cover ครึ่งล่าง ขวาล่างของ sidebar header
    ...
    -- Traffic Lights (อยู่ใน sidebar header)
    ...
    -- Content Header (ส่วนบนของ content - สีขาว)
    ...
    -- Cover ครึ่งล่าง ซ้ายล่างของ content header
    ...
    -- Navigation Buttons (อยู่ใน content header)
    local navContainer = Instance.new("Frame")
    navContainer.Name = "NavigationButtons"
    navContainer.Size = UDim2.new(0, 70, 0, 24)
    navContainer.Position = UDim2.new(0, 16, 0, 14)
    navContainer.BackgroundTransparency = 1
    navContainer.Parent = contentHeader
    
    -- Back Button
    local backButton = Instance.new("TextButton")
    backButton.Name = "BackButton"
    backButton.Size = UDim2.new(0, 28, 0, 24)
    backButton.Position = UDim2.new(0, 0, 0, 0)
    backButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    backButton.BackgroundTransparency = 1
    backButton.BorderSizePixel = 0
    backButton.Text = "<"
    backButton.TextColor3 = MacOSLib.Theme.SecondaryText
    backButton.Font = Enum.Font.GothamBold
    backButton.TextSize = 16
    backButton.AutoButtonColor = false
    backButton.Parent = navContainer
    addCorner(backButton, 6)
    
    -- Forward Button
    local forwardButton = Instance.new("TextButton")
    forwardButton.Name = "ForwardButton"
    forwardButton.Size = UDim2.new(0, 28, 0, 24)
    forwardButton.Position = UDim2.new(0, 36, 0, 0)
    forwardButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    forwardButton.BackgroundTransparency = 1
    forwardButton.BorderSizePixel = 0
    forwardButton.Text = ">"
    forwardButton.TextColor3 = MacOSLib.Theme.SecondaryText
    forwardButton.Font = Enum.Font.GothamBold
    forwardButton.TextSize = 16
    forwardButton.AutoButtonColor = false
    forwardButton.Parent = navContainer
    addCorner(forwardButton, 6)
    
    -- Hover effects for navigation buttons
    for _, navBtn in ipairs({backButton, forwardButton}) do
        navBtn.MouseEnter:Connect(function()
            TweenService:Create(navBtn, TweenInfo.new(0.15), {
    ...
    -- Window Title Text (อยู่ใน content header)
    local titleLabel = Instance.new("TextLabel")
    titleLabel.Name = "TitleLabel"
    titleLabel.Size = UDim2.new(1, -110, 1, 0)
    titleLabel.Position = UDim2.new(0, 95, 0, 0)
    ...
    -- Sidebar ScrollingFrame (ใต้ header)
    ...
    -- Content ScrollingFrame (ใต้ header)
    ...
    -- Make window draggable (รองรับทั้ง PC และมือถือ)
    ...
    function WindowAPI:SaveConfig()
        if not configFileName then return end
        local configData = {}
        ...
        local success = pcall(function()
            ...
            writefile(MacOSLib.ConfigFolder.."/"..configFileName..".json", HttpService:JSONEncode(configData))
        end)
    end
    ...
    function WindowAPI:LoadConfig()
        ...
        local success, data = pcall(function()
            return readfile(MacOSLib.ConfigFolder.."/"..configFileName..".json")
        end)
...
-- สร้าง Sidebar Item (รายการเมนูด้านซ้าย)
function MacOSLib:CreateSidebarItem(parent, config)
    ...
    -- FIX: เพิ่ม method Select แบบถูกต้อง
    item.Select = selectItem
    
    return item
end

-- สร้าง Section Header (หัวข้อหมวดหมู่)
...
-- สร้าง Toggle Switch (สวิตช์เปิด/ปิด)
...
-- สร้าง Text Input (ช่องกรอกข้อความ)
