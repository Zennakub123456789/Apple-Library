...
local ConfigFolder = "MacUI_Config"
local SaveDataCache = {}
...
local function DeleteConfig(idmap)
    local fileName = ConfigFolder .. "/" .. tostring(idmap) .. "_Config.json"
    if isfile(fileName) then
        delfile(fileName)
        return true
...
local function CreateKeySystem(config, callback)
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        ...
        if CheckKey(enteredKey) then
            ...
            if keySettings.SaveKey then
                local tempData = LoadData("_MacUI_KeySystem")
                tempData["_MacUI_SavedKey"] = enteredKey
                SaveData(tempData, "_MacUI_KeySystem")
            end
    ...
    if keySettings.SaveKey then
        local tempData = LoadData("_MacUI_KeySystem")
        local savedKey = tempData["_MacUI_SavedKey"]
...
function MacUI:Window(config)
    ...
    function self:ResetConfig()
        local success = DeleteConfig(self.ConfigID)
        if success then
            SavedData = {}
        end
        return success
    ...
    if config.Discord and config.Discord.Enabled then
        ...
        if discordSettings.RememberJoins then
            local savedJoined = SavedData["_Discord_" .. (discordSettings.Invite or "default")]
            if savedJoined then
                shouldPrompt = false
            end
        end
        ...
        if shouldPrompt and syn and syn.request then
            local success = pcall(function()
                ...
                if discordSettings.RememberJoins then
                    SavedData["_Discord_" .. (discordSettings.Invite or "default")] = true
                    SaveConfigData()
                end
            end)
        end
    ...
    local Themes = {
        Default = {
            ...
            TitleBarLeft = Color3.fromRGB(225, 225, 230),
            TitleBarRight = Color3.fromRGB(240, 240, 245),
            TitleBarDivider = Color3.fromRGB(200, 200, 205),
        ...
        Dark = {
            ...
            Sidebar = Color3.fromRGB(41, 42, 45),
            TitleBar = Color3.fromRGB(44, 45, 49),
            TitleBarLeft = Color3.fromRGB(41, 42, 45),
            TitleBarRight = Color3.fromRGB(36, 36, 38),
            TitleBarDivider = Color3.fromRGB(50, 51, 55),
            Content = Color3.fromRGB(36, 36, 38),
            Accent = Color3.fromRGB(10, 132, 255),
            TabInactive = Color3.fromRGB(41, 42, 45),
            TabActive = Color3.fromRGB(0, 122, 255),
            TextInactive = Color3.fromRGB(155, 155, 160),
            TextActive = Color3.fromRGB(255, 255, 255),
            TabStrokeInactive = Color3.fromRGB(60, 60, 62),
            TabStrokeActive = Color3.fromRGB(0, 122, 255)
        },
        Ocean = {
            Background = Color3.fromRGB(241, 246, 253),
            Sidebar = Color3.fromRGB(233, 239, 248),
            TitleBar = Color3.fromRGB(235, 241, 250),
            TitleBarLeft = Color3.fromRGB(233, 239, 248),
            TitleBarRight = Color3.fromRGB(241, 246, 253),
            TitleBarDivider = Color3.fromRGB(200, 210, 225),
            Content = Color3.fromRGB(241, 246, 253),
            Accent = Color3.fromRGB(64, 160, 255),
            TabInactive = Color3.fromRGB(233, 239, 248),
            TabActive = Color3.fromRGB(0, 122, 255),
            TextInactive = Color3.fromRGB(90, 110, 140),
            TextActive = Color3.fromRGB(255, 255, 255),
            TabStrokeInactive = Color3.fromRGB(200, 215, 235),
            TabStrokeActive = Color3.fromRGB(0, 122, 255)
        }
    }
    ...
    local TitleBarLeft = create("Frame", {
        Parent = TitleBar,
    ...
    create("UICorner", { Parent = TitleBarLeft, CornerRadius = UDim.new(0, 14) })
    
    local TitleBarLeftMask = create("Frame", {
        Parent = TitleBarLeft,
    ...
    local TitleBarLeftBottomMask = create("Frame", {
        Parent = TitleBarLeft,
    ...
    local TitleBarRight = create("Frame", {
        Parent = TitleBar,
        Size = UDim2.new(1, -SidebarWidth, 1, 0),
        Position = UDim2.new(0, SidebarWidth, 0, 0),
        BackgroundColor3 = currentTheme.TitleBarRight,
        BorderSizePixel = 0,
        ZIndex = 10
    })
    create("UICorner", { Parent = TitleBarRight, CornerRadius = UDim.new(0, 14) })
    
    local TitleBarRightMask = create("Frame", {
        Parent = TitleBarRight,
        Size = UDim2.new(1, 0, 0, 14),
        Position = UDim2.new(0, 0, 1, -14),
        BackgroundColor3 = currentTheme.TitleBarRight,
        BorderSizePixel = 0,
    ...
    local TitleBarRightMaskLeft = create("Frame", {
        Parent = TitleBarRight,
    ...
    local TitleBarDivider = create("Frame", {
        ...
        Position = UDim2.new(0, SidebarWidth, 0, 0),
        BackgroundColor3 = currentTheme.TitleBarDivider,
        BorderSizePixel = 0,
        ZIndex = 11
    })

    local NavigationHolder = create("Frame", {
        Parent = TitleBarLeft,
    ...
    create("UICorner", { Parent = BackButton, CornerRadius = UDim.new(0, 4) })
    
    local ForwardButton = create("TextButton", {
        Parent = NavigationHolder,
        Size = UDim2.new(0, 22, 0, 20),
    ...
    create("UICorner", { Parent = ForwardButton, CornerRadius = UDim.new(0, 4) })
    
    BackButton.MouseEnter:Connect(function()
        tween(BackButton, 0.2, { BackgroundColor3 = Color3.fromRGB(200, 200, 205) })
    end)
    BackButton.MouseLeave:Connect(function()
        tween(BackButton, 0.2, { BackgroundColor3 = Color3.fromRGB(210, 210, 215) })
    end)
    
    ForwardButton.MouseEnter:Connect(function()
        tween(ForwardButton, 0.2, { BackgroundColor3 = Color3.fromRGB(200, 200, 205) })
    end)
    ForwardButton.MouseLeave:Connect(function()
        tween(ForwardButton, 0.2, { BackgroundColor3 = Color3.fromRGB(210, 210, 215) })
    end)
    ...
    local titleXPos = config.Icon and config.Icon ~= 0 and 40 or 20
    local TitleText = create("TextLabel", {
        Parent = TitleBarRight,
        Size = UDim2.new(1, -titleXPos - 20, 1, 0),
        Position = UDim2.new(0, titleXPos, 0, 0),
        BackgroundTransparency = 1,
        Text = config.Title or "Software Update",
        TextColor3 = Color3.fromRGB(50, 50, 55),
        Font = Enum.Font.GothamMedium,
        TextSize = 15,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    for i, c in ipairs(colors) do
        ...
        if i == 1 then
            button.MouseButton1Click:Connect(function()
                ...
                local ConfirmGui = create("ScreenGui", {
                    Parent = getSafeParent(),
                    Name = "MacUI_CloseConfirm",
                    ZIndexBehavior = Enum.ZIndexBehavior.Global,
                    ResetOnSpawn = false
                })
                protectGui(ConfirmGui)
                ...
                create("UICorner", { Parent = ConfirmFrame, CornerRadius = UDim.new(0, 14) })
                ...
                local ConfirmText = create("TextLabel", {
                    ...
                    BackgroundTransparency = 1,
                    Text = "Do you want to close MacUI?",
                    TextColor3 = Color3.fromRGB(80, 80, 85),
                    Font = Enum.Font.Gotham,
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                ...
                ConfirmBtn.MouseButton1Click:Connect(function()
                    ...
                    pcall(function()
                        ...
                        for _, child in ipairs(parent:GetChildren()) do
                            if child:IsA("ScreenGui") and child.Name:match("MacUI") then
                                child:Destroy()
                            end
                        end
                    end)
    ...
    if config.ShowToggleButton ~= false then
        local ToggleGui = create("ScreenGui", {
            Parent = getSafeParent(),
            Name = "MacUI_ToggleButton",
            ZIndexBehavior = Enum.ZIndexBehavior.Global,
            ResetOnSpawn = false
        })
        ...
        local ToggleButton = create("ImageButton", {
            ...
            ImageTransparency = (config.ToggleIcon and config.ToggleIcon ~= "") and 0 or 1,
            Visible = true
        })
        create("UICorner", { Parent = ToggleButton, CornerRadius = UDim.new(0, 6) })
        ...
        local FallbackIcon = create("TextLabel", {
            ...
            Font = Enum.Font.GothamBold,
            TextSize = 24,
            Visible = not (config.ToggleIcon and config.ToggleIcon ~= "")
        })
        ...
        UserInputService.InputChanged:Connect(function(input)
            if input == dragInput and dragging then
                ...
                tween(ToggleButton, 0.1, { Position = newPosition })
            end
        end)
    ...
    function self:SelectTab(index)
        print("[MacUI Debug] === SelectTab START ===")
        print("[MacUI Debug] Requested index:", index)
        print("[MacUI Debug] Total tabs:", #self.Tabs)
        
        if not self.Tabs[index] then
            warn("[MacUI] Tab index " .. tostring(index) .. " does not exist")
            print("[MacUI Debug] ERROR: Tab not found!")
            return
        end
        
        print("[MacUI Debug] Tab exists, checking if locked...")
        
        if self.Tabs[index].Locked then
            print("[MacUI Debug] Tab is LOCKED, returning")
            return
        end
        
        print("[MacUI Debug] Tab not locked, proceeding...")
        
        local previousTab = self.SelectedTab
        self.SelectedTab = index
        self.CurrentTab = index
        
        print("[MacUI Debug] Set SelectedTab to:", index)
        ...
        for i, tab in ipairs(self.Tabs) do
            ...
            if tab.CancelHover then
                tab.CancelHover()
            end
            
            if i ~= index and tab.ButtonFrame and typeof(tab.ButtonFrame) == "Instance" then
                tab.ButtonFrame.BackgroundTransparency = 1
                tab.ButtonFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
                
                if not tab.Label then
                    tab.ButtonFrame.TextColor3 = currentTheme.TextInactive
                end
            end
            
            if i ~= index and tab.Label and typeof(tab.Label) == "Instance" then
                tab.Label.TextColor3 = currentTheme.TextInactive
            end
            
            if i ~= index and tab.Icon and typeof(tab.Icon) == "Instance" then
        ...
        print("[MacUI Debug] selectedTab:", selectedTab)
        print("[MacUI Debug] selectedTab.ButtonFrame:", selectedTab.ButtonFrame)
        print("[MacUI Debug] typeof selectedTab.ButtonFrame:", typeof(selectedTab.ButtonFrame))
        
        if selectedTab.Page and typeof(selectedTab.Page) == "Instance" then
            selectedTab.Page.Visible = true
        end
        
        if selectedTab.CancelHover then
            selectedTab.CancelHover()
        end
        
        if selectedTab.ButtonFrame and typeof(selectedTab.ButtonFrame) == "Instance" then
            print("[MacUI Debug] Inside ButtonFrame check - SUCCESS!")
            print("[MacUI Debug] SelectTab called for index:", index)
            print("[MacUI Debug] Setting BackgroundTransparency to 0")
            print("[MacUI Debug] Setting BackgroundColor3 to:", currentTheme.TabActive)
            
            selectedTab.ButtonFrame.BackgroundTransparency = 0
            selectedTab.ButtonFrame.BackgroundColor3 = currentTheme.TabActive
            
            print("[MacUI Debug] After setting - Transparency:", selectedTab.ButtonFrame.BackgroundTransparency)
            print("[MacUI Debug] After setting - Color:", selectedTab.ButtonFrame.BackgroundColor3)
            
            task.spawn(function()
                task.wait(0.01)
                print("[MacUI Debug] Check 1 (0.01s) - Transparency:", selectedTab.ButtonFrame.BackgroundTransparency)
                print("[MacUI Debug] Check 1 (0.01s) - Color:", selectedTab.ButtonFrame.BackgroundColor3)
                selectedTab.ButtonFrame.BackgroundTransparency = 0
                selectedTab.ButtonFrame.BackgroundColor3 = currentTheme.TabActive
                
                task.wait(0.02)
                print("[MacUI Debug] Check 2 (0.03s) - Transparency:", selectedTab.ButtonFrame.BackgroundTransparency)
                print("[MacUI Debug] Check 2 (0.03s) - Color:", selectedTab.ButtonFrame.BackgroundColor3)
                selectedTab.ButtonFrame.BackgroundTransparency = 0
                selectedTab.ButtonFrame.BackgroundColor3 = currentTheme.TabActive
            end)
            
            if not selectedTab.Label then
                selectedTab.ButtonFrame.TextColor3 = currentTheme.TextActive
            end
        else
            print("[MacUI Debug] PROBLEM: ButtonFrame check FAILED!")
            print("[MacUI Debug] selectedTab.ButtonFrame is:", selectedTab.ButtonFrame)
            print("[MacUI Debug] Reason: ButtonFrame is nil or not an Instance")
        end
    ...
    function self:Tab(name, icon)
        ...
        if not isMobile then
            Button.MouseEnter:Connect(function()
                if self.SelectedTab ~= tabIndex then
                    ...
                    currentHoverTween = tween(Button, 0.2, { 
        ...
        Button.MouseButton1Click:Connect(function()
            print("[MacUI Debug] Button clicked! Tab index:", tabIndex, "Tab name:", name)
            if currentHoverTween then
                currentHoverTween:Cancel()
                currentHoverTween = nil
            end
            print("[MacUI Debug] Calling SelectTab for index:", tabIndex)
            self:SelectTab(tabIndex)
            print("[MacUI Debug] SelectTab returned")
        end)
        
        tab.CancelHover = function()
            isHovering = false
            if currentHoverTween then
                currentHoverTween:Cancel()
                currentHoverTween = nil
        ...
        tab.HoverTween = currentHoverTween
        tab.Index = tabIndex
        tab.Name = name
        tab.Locked = false
        ...
        function tab:Input(cfg)
            ...
            local currentValue = SavedData[cfg.Title] or cfg.Default or ""
            
            local holder = create("Frame", {
                Parent = TabPage,
                Size = UDim2.new(1, -20, 0, cfg.Title and 60 or 38),
                BackgroundTransparency = 1
            })
            ...
            inputBox.FocusLost:Connect(function(enterPressed)
                ...
                if cfg.Title then
                    SavedData[cfg.Title] = currentValue
                    SaveConfigData()
                end
        ...
        function tab:Slider(cfg)
            ...
            local value = SavedData[cfg.Title] or cfg.Default
        ...
        function tab:Dropdown(cfg)
            ...
            if cfg.Title and SavedData[cfg.Title] and not multiSelect then
                selectedValue = SavedData[cfg.Title]
                selectedLabel.Text = selectedValue
                
                if cfg.Callback then
                    task.spawn(function()
                        cfg.Callback(selectedValue)
                    end)
                end
            end
        ...
        function tab:Keybind(cfg)
            ...
            UserInputService.InputBegan:Connect(function(input, gameProcessed)
                if waitingForKey and not gameProcessed then
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        if cfg.Title then
                            SavedData[cfg.Title .. "_Keybind"] = currentKey
                            SaveConfigData()
                        end
            ...
            if cfg.Title and SavedData[cfg.Title .. "_Keybind"] then
                currentKey = SavedData[cfg.Title .. "_Keybind"]
                keyBtn.Text = currentKey
            end
        ...
        function tab:ColorPicker(cfg)
            ...
            local function updateColor()
                ...
                if cfg.Title then
                    SavedData[cfg.Title .. "_Color"] = selectedColor
                    SaveConfigData()
                end
            ...
            if cfg.Title and SavedData[cfg.Title .. "_Color"] then
                selectedColor = SavedData[cfg.Title .. "_Color"]
                colorDisplay.BackgroundColor3 = selectedColor
                local r, g, b = selectedColor.R * 255, selectedColor.G * 255, selectedColor.B * 255
                hue, sat, val = RGBToHSV(r, g, b)
                satValPicker.BackgroundColor3 = HSVToRGB(hue, 1, 1)
        ...
        function tab:Code(cfg)
            ...
            local codeBox = create("TextBox", {
                ...
                codeBox:GetPropertyChangedSignal("Text"):Connect(function()
                        if codeBox.Text ~= currentCode then
                ...
                task.spawn(function()
                        while task.wait(0.1) do
                                if codeBox.TextEditable ~= false then
                                        codeBox.TextEditable = false
                                end
                        end
                end)
