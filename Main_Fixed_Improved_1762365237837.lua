...
local function enableAutoTextScaling(textLabel, originalTextSize)
    if not textLabel or not textLabel:IsA("TextLabel") and not textLabel:IsA("TextButton") and not textLabel:IsA("TextBox") then
        return
    end
    
    local function updateTextScaling()
        textLabel.TextScaled = false
        textLabel.TextSize = originalTextSize or textLabel.TextSize
        ...
        if textBounds.X > absoluteSize.X - 10 or textBounds.Y > absoluteSize.Y - 4 then
    ...
    textLabel:GetPropertyChangedSignal("Text"):Connect(updateTextScaling)
    textLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateTextScaling)
    
    task.spawn(updateTextScaling)
end
...
local function CreateKeySystem(config, callback)
    ...
    enableAutoTextScaling(KeyTitle, 20)
    ...
    local KeyInputStroke = KeyInput:FindFirstChildOfClass("UIStroke")
    KeyInput.Focused:Connect(function()
        tween(KeyInput, 0.2, { BackgroundColor3 = Color3.fromRGB(255, 255, 255) })
        if KeyInputStroke then
            tween(KeyInputStroke, 0.2, { Color = Color3.fromRGB(10, 132, 255) })
        end
    end)
    KeyInput.FocusLost:Connect(function()
        tween(KeyInput, 0.2, { BackgroundColor3 = Color3.fromRGB(245, 245, 250) })
        if KeyInputStroke then
            tween(KeyInputStroke, 0.2, { Color = Color3.fromRGB(200, 200, 210) })
        end
    end)
...
function MacUI:Window(config)
    ...
    local Themes = {
        Default = {
            ...
            TitleBarRight = Color3.fromRGB(248, 248, 252),
            ...
            TextInactive = Color3.fromRGB(0, 0, 0),
            TextActive = Color3.fromRGB(255, 255, 255),
            TabStrokeInactive = Color3.fromRGB(200, 200, 205),
            TabStrokeActive = Color3.fromRGB(10, 132, 255),
            TitleText = Color3.fromRGB(50, 50, 55),
            IconColor = Color3.fromRGB(50, 50, 55)
        },
        Dark = {
    ...
    if config.Icon and config.Icon ~= 0 then
        ...
        local IconImage = create("ImageLabel", {
            ...
            BackgroundTransparency = 1,
            Image = iconUrl,
            ImageColor3 = currentTheme.IconColor or Color3.fromRGB(255, 255, 255),
            ZIndex = 12
        })
    end
    ...
    local TitleText = create("TextLabel", {
        ...
        TextColor3 = currentTheme.TitleText or Color3.fromRGB(50, 50, 55),
        Font = Enum.Font.GothamMedium,
        TextSize = 15,
        TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 11
    })
    enableAutoTextScaling(TitleText, 15)
    ...
    for i, c in ipairs(colors) do
        ...
        if i == 1 then
            button.MouseButton1Click:Connect(function()
                ...
                enableAutoTextScaling(ConfirmTitle, 18)
    ...
    TabButtonsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        TabButtons.CanvasSize = UDim2.new(0, 0, 0, TabButtonsLayout.AbsoluteContentSize.Y + 16)
    end)
    ...
    function self:SelectTab(index)
        ...
        if selectedTab.ButtonFrame and typeof(selectedTab.ButtonFrame) == "Instance" then
            tween(selectedTab.ButtonFrame, 0.3, {
                BackgroundTransparency = 0,
        ...
        if selectedTab.Label and typeof(selectedTab.Label) == "Instance" then
            tween(selectedTab.Label, 0.3, {
                TextColor3 = currentTheme.TextActive
            })
        end
        
        if selectedTab.Icon and typeof(selectedTab.Icon) == "Instance" then
            tween(selectedTab.Icon, 0.3, {
                ImageColor3 = currentTheme.TextActive
            })
        end
    ...
    function self:Tab(name, icon)
        ...
        function tab:Button(cfg)
            ...
            enableAutoTextScaling(titleLabel, 14)
        ...
        function tab:Input(cfg)
            ...
            local inputBox = create("TextBox", {
                Parent = holder,
                Size = cfg.Title and UDim2.new(1, -150, 1, 0) or UDim2.new(1, 0, 1, 0),
                Position = cfg.Title and UDim2.new(0, 150, 0, 0) or UDim2.new(0, 0, 0, 0),
                BackgroundColor3 = Color3.fromRGB(245, 245, 250),
                BorderSizePixel = 0,
                Text = currentValue,
            ...
            local inputBoxStroke = inputBox:FindFirstChildOfClass("UIStroke")
            inputBox.Focused:Connect(function()
                tween(inputBox, 0.2, { BackgroundColor3 = Color3.fromRGB(255, 255, 255) })
                if inputBoxStroke then
                    tween(inputBoxStroke, 0.2, { Color = Color3.fromRGB(10, 132, 255), Thickness = 2.5 })
                end
            end)
            
            inputBox.FocusLost:Connect(function(enterPressed)
                tween(inputBox, 0.2, { BackgroundColor3 = Color3.fromRGB(245, 245, 250) })
                if inputBoxStroke then
                    tween(inputBoxStroke, 0.2, { Color = Color3.fromRGB(200, 200, 210), Thickness = 2 })
                end
        ...
        function tab:Code(cfg)
            ...
            enableAutoTextScaling(titleLabel, 12)
...
function MacUI:Dialog(cfg)
    ...
    enableAutoTextScaling(Title, 18)
