...
function MacUI:Window(config)
    ...
    create("UIStroke", { Parent = MainFrame, Color = Color3.fromRGB(255, 255, 255), Thickness = 2, Transparency = 0.3 })
    ...
    function self:Tab(name, icon)
        ...
        function tab:Dropdown(cfg)
                ...
                for _, option in ipairs(cfg.Options) do
                    ...
                    if multiSelect then
                        ...
                        optionBtn.MouseButton1Click:Connect(function()
                            ...
                            if cfg.Title then
                                SavedData[cfg.Title] = selectedValues
                                SaveConfigData()
                            end
            ...
            if cfg.Title and SavedData[cfg.Title] then
                if multiSelect then
                    if type(SavedData[cfg.Title]) == "table" then
                        selectedValues = SavedData[cfg.Title]
                        selectedLabel.Text = #selectedValues > 0 and table.concat(selectedValues, ", ") or "Select"
                        
                        for _, child in ipairs(optionsList:GetChildren()) do
                            if child:IsA("TextButton") then
                                for _, v in ipairs(selectedValues) do
                                    if v == child.Text then
                                        local checkbox = child:FindFirstChildWhichIsA("Frame")
                                        if checkbox then
                                            checkbox.BackgroundColor3 = currentTheme.Accent
                                            local checkmark = checkbox:FindFirstChildWhichIsA("TextLabel")
                                            if checkmark then
                                                checkmark.Visible = true
                                            end
                                        end
